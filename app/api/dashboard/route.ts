...
type CenterRow = RowDataPacket & { id: number; code: string; name: string; location: string | null; sales: number | string; orders: number; status: string }
...
export async function GET() {
  ...
  try {
    ...
    const centers = await query<CenterRow[]>(`SELECT f.id, f.code, f.name, CONCAT(COALESCE(l.district,''), IF(l.district IS NULL,'',', '), COALESCE(l.division,'')) location, COALESCE(SUM(CASE WHEN o.status <> 'cancelled' THEN o.total ELSE 0 END),0) sales, COUNT(o.id) orders, f.status FROM franchises f LEFT JOIN locations l ON l.id=f.location_id LEFT JOIN orders o ON o.franchise_id=f.id GROUP BY f.id ORDER BY sales DESC LIMIT 5`)
